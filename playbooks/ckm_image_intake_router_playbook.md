...
The image-recognition node must understand image content and visible text regardless of source language, but it always emits English `item_name`, lowercase English canonical `normalized_name`, item boundaries, and amounts. It must not receive or apply `output_locale`. A downstream localization node translates only `item_name` one-to-one after recognition and validation. Localization must never merge, split, add, remove, reorder, or otherwise rewrite intake items. Keep `nutrition_relevant_cues` in concise English and preserve screenshot text in its original language when returning `extracted_text`. For localized `en-US`, `de-DE`, `fr-FR`, and `es-ES` names, start with an uppercase letter and use natural sentence-style casing, never Title Case for every word. Preserve required local capitalization such as German nouns. For `zh-CN`, use natural Simplified Chinese naming.
