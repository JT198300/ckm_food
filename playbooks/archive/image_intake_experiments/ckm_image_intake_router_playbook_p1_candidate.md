# CKM Image Intake Router Playbook v0.2-candidate
...
Before returning any failed result, perform this positive-evidence routing check in order:

1. If the image contains readable food-log or meal-description text, route to `extracted_food_text`.
2. If it contains a recognizable food product, ingredient package, nutrition label, or grocery food, route to `product_or_package`.
3. If it contains any recognizable served, plated, cooked, raw, or prepared edible region, route to `meal_photo`.
4. Only then consider fasting, non-food, unsupported, or uncertain failure routes.

Food evidence takes precedence over background size. Do not return `non_food_image_detected` merely because food occupies a small part of the image. When an edible region is clear but its exact subtype is not, return a useful generic food class with low confidence instead of failing.
...
For every visible edible food item, output a rough grams/ml estimate when a usable scale anchor exists.

Use this amount decision algorithm:

```python
def estimate_amount(item, image):
    if readable_net_weight_or_capacity(item):
        return package_amount(), "package_label"
    if visible_count(item):
        return convert_count_to_typical_total_grams_or_ml(item), "visual_estimate"
    if has_scale_anchor(item, image):
        return rough_central_estimate(item), "visual_estimate"
    return None, "unknown"
```

Usable scale anchors include a standard plate, bowl, cup, glass, can, bottle, pan, tray, package, hand, utensil, or a clearly countable serving. A complete plated meal, whole pizza, steak, patty, egg, bread slice, cheese portion, dessert piece, or filled beverage container normally has enough evidence for a rough estimate.

Prefer an imperfect but plausible central estimate over null. Normal visual error of tens of grams is acceptable because the user can edit the amount. Express weak portion evidence with low confidence; do not use null merely because the amount is not precise.

Use null only when there is no usable scale anchor, the edible region is severely cropped or obscured, the container contents or fill level are not visible, or plausible amounts differ by roughly an order of magnitude.

If the food identity is useful but the amount still cannot be estimated, keep the food item with `estimated_amount = null`, `amount_source = "unknown"`, and low confidence. The business UI can ask the user to enter or correct the amount before consumed nutrition calculation. Missing amount must not prevent per-100g nutrition or keto-label analysis.
...
## Beverage And Supplement Boundaries

- A visibly non-empty served beverage is valid intake. Include coffee, tea, milk drinks, soft drinks, alcohol, smoothies, shakes, and similar beverages when the content or product is recognizable; estimate `ml` from the vessel and visible fill level.
- Do not infer a beverage from an empty cup, closed opaque container with no intake evidence, or a vessel whose contents are not visible.
- Plain water may be omitted from nutrition intake unless it is explicitly logged or is materially combined with another consumed ingredient.
- Pills, medication, vitamin bottles, dietary-supplement tubs, creatine products, electrolyte capsules, and energy-shot products are not food items for this workflow. Return `failed` with `unsupported_input_type` when they are the only recognizable content.
- Food ingredients such as butter, olive oil, coconut oil, MCT oil, protein foods, and clearly prepared meal-replacement drinks remain valid when visibly served, used in the meal, or explicitly logged. A supplement package alone is not proof of consumption.
