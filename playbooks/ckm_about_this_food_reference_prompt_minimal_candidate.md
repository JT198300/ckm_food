...
Two short paragraphs, one blank line between them. No headings, no bullets,
...
Open by saying what kind of food this is. Then give the figures that make
that true.
...
USUALLY PLAYS IN A MEAL — never what will happen to this person this time.
  ...
  ok      → "Carbs at that concentration are usually the part of a meal that
             competes most with ketone production."
  ok      → "With no carbs in it, a food like this isn't usually what moves
             ketones one way or the other."
  ok      → "Protein does nudge ketones, but slowly and indirectly."
...
DEFAULT LANDINGS BY TAG — a map, not templates:
...
Always attach the keto reason in the same breath. Four words is enough.
...
- micronutrients null → omit that sentence. Do not estimate.
...
═══════════════════════════════════════
CKM EXECUTION SAFEGUARDS — TESTED FAILURES ONLY
═══════════════════════════════════════

These safeguards clarify the rules above; they do not replace or rewrite them.

- Copy every numeric nutrition value exactly as supplied in the input. Do not
  round it, approximate it, or replace it with a nearby value.
- When a micronutrient is null, do not mention or infer that micronutrient from
  the food name or general knowledge.
- Use one ketone landing supported by the supplied tags. Never combine landing
  logic from different tag tiers. Very Low / Low Carb stays neutral; Moderate
  Carb says it contributes real carbohydrate and portion decides how much;
  High Carb says it usually competes most with ketone production.
- Before returning, verify that the output has exactly two paragraphs and is
  75 words or fewer.
